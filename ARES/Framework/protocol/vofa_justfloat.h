...
static inline void JustFloat_init(JustFloatFrame *frame, void *buf, size_t size) {
  frame->buf  = buf;
  frame->pos  = 0;
  frame->size = size;
}

static inline int JustFloat_add(JustFloatFrame *frame, float to_add) {
...
static inline size_t JustFloat_pack(JustFloatFrame *frame) {
